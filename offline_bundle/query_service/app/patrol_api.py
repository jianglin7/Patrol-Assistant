...
from app.semantic_report import compose_semantic_report
...
def _classify_bucket_with_llm(question: str, trace_id: str | None = None) -> dict[str, Any] | None:
    ...
    if not parsed:
        trace_emit(trace_id, "llm_intent_json_not_found", preview=content[:180])
        return None
...
def _build_llm_clarification_response(
    *,
    ...
    parsed = _parse_first_json_object(content)
    if not parsed:
        trace_emit(trace_id, "llm_clarification_json_not_found", preview=content[:180])
        return None
    ...
    *,
    ...
    try:
        if bucket == "realtime_patrol":
            data = get_realtime_patrol_brief(tenant_id, synthesize=True, query_options=query_options)
        elif bucket == "daily_patrol":
            data = get_daily_patrol_brief(tenant_id, synthesize=True, query_options=query_options)
        elif bucket == "realtime_warning":
            data = get_realtime_warning_brief(tenant_id, synthesize=True, query_options=query_options)
        elif bucket == "daily_warning":
            data = get_daily_warning_brief(tenant_id, synthesize=True, query_options=query_options)
...
def route_assistant_query(
    ...
    if bucket != "general":
        ...
        clarification = (
            ...
            or _build_clarification_response(question=text, bucket=bucket, query_options=query_options)
        )
        ...
        if _should_prefer_semantic_report(text, bucket):
            ...
            if semantic_hit is not None:
                ...
                return semantic_hit
        hit = _try_simple_bucket_answer(tenant_id, bucket, query_options=query_options)
        if hit is not None:
    ...
    if bucket == "general" and settings.assistant_auto_plan_enabled:
        clarification = (
            ...
            or _build_clarification_response(question=text, bucket=bucket, query_options=raw_query_options)
        )
        if clarification is not None:
            trace_emit(
                trace_id,
        ...
        if auto_bucket != "general" and auto_conf >= min_conf:
            ...
            auto_hit = _try_simple_bucket_answer(tenant_id, auto_bucket, query_options=auto_options)
    ...
    if llm_route:
        ...
        if llm_bucket != "general" and llm_confidence >= float(settings.assistant_intent_llm_min_confidence):
            ...
            llm_hit = _try_simple_bucket_answer(tenant_id, llm_bucket, query_options=query_options)
    ...
    if followup_refine and context_bucket:
        ...
        ctx_hit = _try_simple_bucket_answer(tenant_id, context_bucket, query_options=fallback_options)
