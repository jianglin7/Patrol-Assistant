...
def ask_demo_assistant(question: str) -> dict[str, Any]:
    ...
    if any(keyword in text for keyword in ("实时课堂", "正在上课", "实时AI巡查", "实时AI巡课")):
        return _realtime_patrol_response(text)

    if any(keyword in text for keyword in ("非课表", "今日已经上课", "今日已上课", "今日课堂AI巡查")):
    ...
    if any(keyword in text for keyword in ("今日课堂AI预警", "今日预警", "已结束上课", "今日已完成AI巡查")):
...
def _stats_response(question: str) -> dict[str, Any]:
    ...
    return {
        ...
        "answer": (
            ...
            "这说明当前波动主要集中在本科阶段，尤其是基础课和大班课场景，问题不在单个节次而在教学组织与学习节奏匹配。\n"
...
def _realtime_patrol_response(question: str) -> dict[str, Any]:
    ...
    return {"success": True, "intent": "课表时间段智能AI巡课", "answer": answer, "data": [], "generated_at": _now()}
...
def _daily_patrol_response(question: str) -> dict[str, Any]:
    ...
    return {"success": True, "intent": "非课表时间段智能AI巡课", "answer": answer, "data": [], "generated_at": _now()}
...
def _realtime_warning_response(question: str) -> dict[str, Any]:
    ...
    answer = (
        ...
        f"实时AI巡查轮次进度：{round_progress}，上课节次：{section}，当前上课课堂：{class_count} 个。\n"
        f"预警课堂：{warning_count} 个，占比 {warning_ratio}% ，整体预警风险等级：{risk}。\n"
        f"预警课堂类型：{warning_types}。\n"
        "结论：当前风险可控但有上行迹象，建议先对风险课堂做即时提醒并加密下一轮AI巡查。"
    )
...
def _daily_warning_response(question: str) -> dict[str, Any]:
    ...
    answer = (
        ...
        f"今日已完成AI巡查课堂：{inspected} 个，预警课堂：{warning} 个，占比 {ratio}% 。\n"
