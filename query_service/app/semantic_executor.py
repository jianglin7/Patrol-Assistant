...
_FRIENDLY_ERROR_MESSAGES = {
    "E_TEMPLATE_NOT_FOUND": "查询模板未找到或未启用。",
    "E_TEMPLATE_PARAM_MISSING": "查询参数不完整，请补充必要条件后重试。",
    "E_FILTER_CONFLICT": "查询参数格式不正确，请调整筛选条件后重试。",
    "E_SQL_READONLY_BLOCKED": "当前只允许执行只读查询。",
    "E_DB_QUERY_FAILED": "查询执行失败，请检查查询条件或稍后重试。",
    "E_DB_CONNECT_FAIL_BUSINESS": "业务数据源暂时无法连接，请稍后重试。",
    "E_DB_QUERY_TIMEOUT": "查询超时，请缩小查询范围或稍后重试。",
    "E_INTERNAL": "查询执行异常，请稍后重试。",
}

_DB_ERROR_CODES = {"E_DB_QUERY_FAILED", "E_DB_CONNECT_FAIL_BUSINESS", "E_DB_QUERY_TIMEOUT"}


def _safe_error_message(code: str, message: str | None = None) -> str:
    if code in _DB_ERROR_CODES:
        return _FRIENDLY_ERROR_MESSAGES.get(code, "查询执行失败，请稍后重试。")
    if message:
        return redact_sensitive_text(message, max_len=200)
    return _FRIENDLY_ERROR_MESSAGES.get(code, code)
...
def _error_meta(code: str, message: str | None = None) -> dict[str, Any]:
    ...
    return {
        "code": code,
        "message": _safe_error_message(code, message or str(meta.get("message") or "")),
...
def _map_exception(exc: Exception) -> dict[str, Any]:
    ...
    if "connect" in lowered or "connection" in lowered:
        return _error_meta("E_DB_CONNECT_FAIL_BUSINESS")
    if "timeout" in lowered:
        return _error_meta("E_DB_QUERY_TIMEOUT")
    return _error_meta("E_DB_QUERY_FAILED")
